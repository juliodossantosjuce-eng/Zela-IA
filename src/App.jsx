...
const SERVICES = [
  ...
  },
  {
    ...
    placeholder: "Ex: Traduza para inglês: 'Angola é um país cheio de oportunidades e recursos naturais.'",usos: ["PT → EN", "EN → PT", "Documentos oficiais", "Textos técnicos"],
...
export default function App() {
  ...
  const sv = SERVICES.find(s => s.id === servicoId);useEffect(() => { const t = setInterval(() => setCount(n => n + Math.floor(Math.random() * 3)), 9000); return () => clearInterval(t); }, []);
  ...
  function gerarPedidoId() {
    return "ZL" + Date.now().toString().slice(-6);
  }
  ...
  function abrirWhatsApp() {
    ...
    const url = `https://wa.me/${OWNER.whatsapp}?text=${encodeURIComponent(msg)}`;
  ...
  const css = `
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    @keyframes 
      ...
      setCodigoErro(true);
      setTimeout(() => setCodigoErro(false), 2500);
    ...
    textarea:focus,input:focus{outline:none!important;border-color:#FF6B3566!important}button{cursor:pointer;border:none;background:none;font-family:inherit}
  ...
  if (tela === "landing") return (
    <div style={st.root}>
      ...
      <section style={{padding:"52px 24px 0",position:"relative",zIndex:1,animation:"fadeUp .6s ease both"}}><div style={st.heroBadge}>🚀 A primeira plataforma de IA em Kwanzas</div>
        <h1 style={st.h1}>
          Inteligência<br/>
          <span style={st.h1Grad}>Artificial</span><br/>
          <span style={{color:"#444",fontSize:30}}>ao seu serviço</span>
        </h1>
      ...
      <section style={{padding:"44px 24px 0",position:"relative",zIndex:1}}>
        <div style={st.tag}>Nossos Serviços</div><h2 style={st.h2}>O que fazemos por si</h2>
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          {SERVICES.map((s,i)=>(
            <div key={s.id} className="card" style={{background:"#0d0d1a",border:"1px solid #1a1a2e",borderRadius:16,overflow:"hidden",cursor:"pointer",animation:`fadeUp .5s ${i*.08}s ease both`}}
              onClick={()=>{setServicoId(s.id);setTela("pedido")}}>
              ...
              <div style={{padding:"18px 18px 20px"}}>
                ...
                <button className="shine" style={{background:s.gradient,color:"#fff",fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:13,padding:"9px 18px",borderRadius:8,border:"none",cursor:"pointer"}}>
                  Usar agora →
                </button>
              </div>
            </div>
      ...
      <section style={{padding:"44px 24px 0",position:"relative",zIndex:1}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {[
            {n:"01",icon:"🎯",t:"Escolha",d:"Seleccione o serviço que precisa",c:"#FF6B35"},
            {n:"02",icon:"💬",t:"Descreva",d:"Explique o que quer com detalhes",c:"#00C9FF"},
            {n:"03",icon:"💳",t:"Pague",d:"Transferência Multicaixa + comprovativo WhatsApp",c:"#A8FF78"},
            {n:"04",icon:"✨",t:"Receba",d:"Código confirmado → resultado em segundos",c:"#C77DFF"},
          ].map(p=>(
      ...
      <footer style={{padding:"32px 24px",display:"flex",flexDirection:"column",alignItems:"center",gap:6,borderTop:"1px solid #13131f",marginTop:44,position:"relative",zIndex:1}}>
        ...
        </div><span style={{color:"#333",fontSize:12}}>🇦🇴 Feito para Angola · Pague em Kwanzas</span>
      </footer>
    </div>
  );
    ...
    <div style={st.root}>
      ...
      <div style={{...st.page,animation:"fadeUp .4s ease both"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {SERVICES.map(s=>(<div key={s.id} className="card" style={{background:"#0d0d1a",border:"1px solid #1a1a2e",borderRadius:14,padding:"16px",display:"flex",alignItems:"center",gap:14,cursor:"pointer",position:"relative",overflow:"hidden"}}
              onClick={()=>{setServicoId(s.id);setTela("pedido")}}>
      ...
      <div style={{...st.page,animation:"fadeUp .4s ease both"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          <button className="shine"
            style={{...st.btnMain,background:(!pedido.trim()||!nome.trim()||!telefone.trim())?"#1a1a2e":sv.gradient,color:(!pedido.trim()||!nome.trim()||!telefone.trim())?"#444":"#fff",width:"100%",padding:"15px",fontSize:16,justifyContent:"center",cursor:(!pedido.trim()||!nome.trim()||!telefone.trim())?"not-allowed":"pointer"}}
    ...
    <div style={st.root}>
      ...
      <div style={{...st.page,animation:"fadeUp .4s ease both"}}>
        ...
        <div style={{background:"#0d0d1a",border:"1px solid #1a1a2e",borderRadius:16,padding:20,marginBottom:14}}>
          ...
          {[
            {l:"Banco", v:MULTICAIXA.banco},
            {l:"Titular", v:MULTICAIXA.titular},
            {l:"IBAN", v:MULTICAIXA.iban, mono:true},
            {l:"Referência", v:`${MULTICAIXA.referencia}-${pedidoId}`},
          ].map(r=>(
        ...
        <div style={{background:"#0d1a0d",border:"1px solid #25D36625",borderRadius:16,padding:18,marginBottom:14}}>
          ...
          <p style={{color:"#aaa",fontSize:13,lineHeight:1.6,marginBottom:14}}>
            Após transferir, clique no botão abaixo para enviar o comprovativo ao Júlio via WhatsApp. Ele vai responder com o seu <strong style={{color:"#fff"}}>código de 4 dígitos</strong>.
          </p>
          <button className="shine"
            style={{background:"linear-gradient(135deg,#25D366,#128C7E)",color:"#fff",fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:14,padding:"13px 20px",borderRadius:10,border:"none",cursor:"pointer",width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}
        ...
        <div style={{background:"#0d0d1a",border:"1px solid #1a1a2e",borderRadius:16,padding:18,marginBottom:14}}>
          ...
          <p style={{color:"#aaa",fontSize:13,lineHeight:1.5,marginBottom:14}}>
            Após o Júlio confirmar o pagamento, insira o código de 4 dígitos que ele lhe enviou.
          </p>
          <input
            value={codigoInput}
            ...
            placeholder="0000"
            maxLength={4}
            className={codigoErro?"shake":""}style={{...st.input,fontSize:28,fontWeight:800,textAlign:"center",letterSpacing:12,fontFamily:"'Syne',sans-serif",padding:"14px",borderColor:codigoErro?"#ff4444":"#1a1a2e",color:codigoErro?"#ff4444":"#F0EDE8"}}
          />
        ...
        <button className="shine"
          style={{...st.btnMain,background:codigoInput.length===4?"linear-gradient(135deg,#C77DFF,#FFB347)":"#1a1a2e",color:codigoInput.length===4?"#fff":"#444",width:"100%",padding:"16px",fontSize:16,justifyContent:"center",cursor:codigoInput.length===4?"pointer":"not-allowed"}}
          disabled={codigoInput.length!==4} onClick={verificarCodigo}>
          ✅ Confirmar código e obter resultado
        </button><div style={{background:"#FF6B3510",border:"1px solid #FF6B3530",borderRadius:10,padding:"12px 14px",display:"flex",gap:10,marginTop:12}}>
          <span>⚠️</span>
  ...
  if (tela === "resultado" && sv) return (
    <div style={st.root}>
      ...
      <div style={{...st.page,animation:"fadeUp .4s ease both"}}><div style={{background:sv.gradient,borderRadius:14,padding:"14px 16px",display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
          ...
          {!loading && resposta && (
            <button onClick={()=>{navigator.clipboard.writeText(resposta);setCopiado(true);setTimeout(()=>setCopiado(false),2000)}}
              style={{background:"rgba(255,255,255,.2)",color:"#fff",borderRadius:8,padding:"6px 14px",fontSize:12,border:"none",cursor:"pointer",fontWeight:600,flexShrink:0}}>
        ...
        <div style={{background:"#0d0d1a",border:"1px solid #1a1a2e",borderRadius:16,overflow:"hidden",marginBottom:20}}>
          <div style={{padding:"12px 16px",borderBottom:"1px solid #13131f",background:"#0a0a15",display:"flex",alignItems:"center",gap:8}}>
            {loading? <><div style={{width:7,height:7,borderRadius:"50%",background:sv.cor,animation:"bounce 1.2s 0s infinite ease-in-out"}}/><div style={{width:7,height:7,borderRadius:"50%",background:sv.cor,animation:"bounce 1.2s .2s infinite ease-in-out"}}/><div style={{width:7,height:7,borderRadius:"50%",background:sv.cor,animation:"bounce 1.2s .4s infinite ease-in-out"}}/><span style={{color:"#555",fontSize:13,marginLeft:4}}>A IA está a trabalhar...</span></>
              : <span style={{color:"#A8FF78",fontSize:13,fontWeight:600}}>✅ Resultado gerado com sucesso</span>
            }
          </div>
          <div style={{padding:18,minHeight:160}}>
            {loading
              ? <div style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"28px 0"}}><Spinner color={sv.cor}/><p style={{color:"#444",fontSize:13,marginTop:14,textAlign:"center"}}>Processando com IA avançada...</p></div>: <pre style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:14,lineHeight:1.75,color:"#C0BCB6",whiteSpace:"pre-wrap",wordBreak:"break-word"}}>{resposta}</pre>
            }
          </div>
        </div>
